...
class FCLCNPDistractor(nn.Module):
    """
    Conditional Neural Process for Distractor with Functional Contrastive Learning
    """
    def __init__(self, config):
        super(FCLCNPDistractor, self).__init__()
    ...
    def forward(self, batch_train_images, label_train, batch_test_images, label_test, test=False):
        ...
        if self.ctx_num:
            ...
            z = torch.ones(self.task_num, self.test_num, 256).to(self.device) * 0.0
            # log_variance = torch.ones(self.task_num, 1, 256).to(self.device) * 1.0

        if not test:
            # used for contrastive loss
            x_qry = batch_test_images.reshape(-1, self.img_channels, self.img_size[0], self.img_size[1])
            x_qry = self.img_encoder(x_qry)
            label_test = self.transform_y(label_test)
            x_q = torch.cat([x_qry, label_test], dim=2)
            rq = self.task_encoder(x_q)
            if self.agg_mode == 'mean':
                rq = torch.mean(rq, dim=1, keepdim=False)
                z_q = self.mu(rq)
            elif self.agg_mode == 'max':
                rq = torch.max(rq, dim=1, keepdim=False)[0]
                z_q = self.mu(rq)
            elif self.agg_mode == 'baco':
                rq = self.latent_mu(rq)
                log_variance_rq = self.latent_var(rq)
                log_variance_rq = 1e-5 + F.softplus(log_variance_rq)
                rq, log_variance_rq = self.baco(rq, log_variance_rq)
                z_q = self.mu(rq)
        ...
        generated_angles, generated_var = self.decoder(batch_test_images, z)
